...
export const state = () => ({
  ...
  token_type: localStorage.getItem('token_type') || 'JWT',
  refresh: null,
  login_error:{}
});
...
export const actions = {
  ...
  async login({ commit }, form) {
    return new Promise((resolve, reject)=>{
      this.$axios.$post("/api/auth/jwt/create/", form).then((res)=>{
        commit("SET_AUTH", {token: res.access, token_type: "JWT"});
        console.log("JWT", res.access)
        this.$axios.defaults.headers.common['Authorization'] = "JWT " + res.access
  ...
  async loginGoogle({ commit }, data) {
    ...
    const user = await this.$axios.$get("/api/auth/users/me/");
